...
ims_paths = np.array(load_obj.img_files)
labs_paths = np.array(load_obj.label_files)
# %%
...
for i, (im, lab) in enumerate(zip(*res)):
	cv2.imwrite(os.path.join(tg_dir, 'images/train', f"{prefix}{i:03d}.jpg"), im)
	_lab = lab.copy().astype(float)
	_lab[:, 1:] = general.xyxy2xywhn(_lab[:, 1:], im.shape[1], im.shape[0])
	_lab[:, 0] = _lab[:, 0].astype(int)
	# print(_lab)
	with open(os.path.join(tg_dir, 'labels/train', f"{prefix}{i:03d}.txt"), 'w') as fw:
