...
class GenImagesAndLabels(Dataset):  # for training/testing
    ...
    def gen(self):
        ...
        for index in range(len(self.img_files)):
            ...
            #print(index)
            os.makedirs("../data_agu/images/img4/", exist_ok=True)
            os.makedirs("../data_agu/labels/img4/", exist_ok=True)
            cv2.imwrite("../data_agu/images/img4/"+str(index)+".jpg",img)
            txt_file=open("../data_agu/labels/img4/"+str(index)+".txt",'w')
            for item in labels:
                item=item.tolist()
                item[0]=int(item[0])
                string =""
